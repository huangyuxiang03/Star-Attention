...
model, tokenizer = load_ring_model_tokenizer("/home/test/test01/hyx/Llama-3.2-1B-instruct")
...
if rank == 0:
    ...
    model = AutoModelForCausalLM.from_pretrained("/home/test/test01/hyx/Llama-3.2-1B-instruct", attn_implementation='flash_attention_2', torch_dtype=torch.float16, device_map=torch.device(f"cuda:{rank}"))
    with torch.no_grad():
        output = model(input_ids, position_ids=position_ids, output_hidden_states=True)
    hs = hs[-1]
